...
import { blurBackground } from "@/services/api";
...
const BLUR_STATUS = [
  ...
  { threshold: 25, text: "Detecting subject..." },
  { threshold: 70, text: "Blurring background..." },
  { threshold: 100, text: "Complete!" },
];

export function BlurBackgroundUpload({
  config,
  registerLoadDemo,
}: {
  config: Pick<
  ...
  const runProcessing = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await blurBackground(file, intensity);
        completeProgress();
        try {
    ...
    [clearPreviewBlob, completeProgress, intensity, resetProgress, showToast, startProgress]
  );
  ...
  const downloadProcessed = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${fileInfo?.name?.replace(/\.[^.]+$/, "") || "image"}-blur-bg.jpg`;
      a.click();
      ...
      >
        <div>
          ...
          <p className="mt-3 max-w-lg text-sm leading-relaxed text-white/50">
            Drop a photo below. Subject stays clear, only background gets blurred.
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/35">
            <span className="inline-flex items-center gap-1.5">
              <Zap className="h-3.5 w-3.5 text-brand-secondary" /> ~2–3s typical
            </span>
          ...
          <div className="mt-6 border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="mt-3 flex items-center gap-4">
              ...
              />
              <span className="w-12 text-right text-sm font-semibold text-white">{intensity}</span>
          ...
          <div className="mt-8">
                ...
                <AnimatePresence mode="wait">
                  {!showUploadContent ? (
                    ...
                    <motion.div key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                      ...
                      <ProcessingAnimation progress={progress} active statusMessages={BLUR_STATUS} />
                    </motion.div>
                    ...
                    <motion.div key="uploaded" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-5 py-4 sm:flex-row sm:items-start">
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        ...
                        {fileInfo && <p className="mt-3 text-sm text-white/45">{fileInfo.name} · {formatBytes(fileInfo.size)} · intensity {intensity}</p>}
            ...
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-xs text-white/40">
              ...
              <span className="inline-flex items-center gap-1.5">
                <CheckCircle2 className="h-3.5 w-3.5 text-brand-secondary" /> Subject stays clear
              </span>
              <span className="inline-flex items-center gap-1.5">
        ...
        {state === "success" && originalUrl && processedUrl && fileInfo && (
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <ResultSection
              ...
              resultAriaLabel="Background blur results"
              processedLabel={`Blurred Background (${intensity})`}
              originalAlt="Original uploaded image before background blur"
              processedAlt="Image with subject clear and background blurred"
