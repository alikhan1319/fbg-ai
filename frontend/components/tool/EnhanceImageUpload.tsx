...
import { ArrowRight, CheckCircle2, Info, Lock, Timer, Upload, Zap } from "lucide-react";
...
const ENHANCE_STATUS = [
  { threshold: 0, text: "Uploading image..." },
  { threshold: 15, text: "Analyzing blur and noise..." },
  { threshold: 35, text: "Applying professional clarity..." },
  { threshold: 70, text: "Refining color and detail..." },
  { threshold: 100, text: "Complete!" },
];

export function EnhanceImageUpload({
  config,
  registerLoadDemo,
}: {
  config: Pick<
  ...
  const runProcessing = useCallback(
    async (file: File) => {
      ...
      if (!okType) return void showToast("Only JPG, PNG, or WebP files are supported.");
      if (file.size > max) return void showToast(`Max file size is 15MB. Your file is ${formatBytes(file.size)}.`);
      ...
      try {
        const result = await enhanceImage(file, { sharpen, denoise });
        completeProgress();
        try {
    ...
    [clearPreviewBlob, completeProgress, denoise, resetProgress, sharpen, showToast, startProgress]
  );
  ...
  const downloadProcessed = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${fileInfo?.name?.replace(/\.[^.]+$/, "") || "image"}-enhanced.jpg`;
      a.click();
      ...
      >
        <div>
          ...
          <p className="mt-3 max-w-lg text-sm leading-relaxed text-white/50">
            Run advanced enhancement with AI clarity recovery, denoise cleanup, and controlled sharpening.
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/35">
            <span className="inline-flex items-center gap-1.5">
              <Zap className="h-3.5 w-3.5 text-brand-secondary" /> ~5–40s typical
            </span>
          ...
          <div className="mt-6 grid gap-4 border border-white/10 bg-white/[0.04] p-4 sm:grid-cols-2">
            <div>
              ...
              <div className="mt-2 flex items-center gap-4">
                <input type="range" min={0} max={100} value={sharpen} disabled={uploadLocked} onChange={(e) => setSharpen(Number(e.target.value))} className="w-full accent-brand-secondary" aria-label="Sharpen" />
                <span className="w-10 text-right text-sm font-semibold text-white">{sharpen}</span>
              </div>
            </div>
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-white/35">Denoise</p>
              <div className="mt-2 flex items-center gap-4">
                <input type="range" min={0} max={100} value={denoise} disabled={uploadLocked} onChange={(e) => setDenoise(Number(e.target.value))} className="w-full accent-brand-accent" aria-label="Denoise" />
            ...
            <div className="sm:col-span-2">
              <p className="flex items-center gap-2 text-xs text-white/40">
                ...
                Recommended: Sharpen 35-50, Denoise 20-35 for professional results.
              </p>
            </div>
          </div>
          ...
          <div className="mt-8">
              ...
              <div className="p-5 sm:p-7">
                <AnimatePresence mode="wait">
                  {!showUploadContent ? (
                      ...
                      {localPreview && (
                        <div className="flex items-center gap-4 border border-white/10 bg-white/[0.04] p-3">
                          ...
                          <div className="min-w-0 text-left">
                            ...
                            {fileInfo && <p className="text-xs text-white/40">{formatBytes(fileInfo.size)}</p>}
                            <p className="mt-1 text-xs text-brand-secondary">{config.processingLabel}</p>
                          </div>
                        </div>
                      )}
                      <ProcessingAnimation progress={progress} active statusMessages={ENHANCE_STATUS} />
                    </motion.div>
                    ...
                    <motion.div key="uploaded" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-5 py-4 sm:flex-row sm:items-start">
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        ...
                        {fileInfo && <p className="mt-3 text-sm text-white/45">{fileInfo.name} · {formatBytes(fileInfo.size)} · sharpen {sharpen} · denoise {denoise}</p>}
                        <button type="button" onClick={reset} className="mt-4 text-sm font-medium text-brand-secondary underline-offset-2 hover:underline">
            ...
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-xs text-white/40">
              ...
              <span className="inline-flex items-center gap-1.5">
                <CheckCircle2 className="h-3.5 w-3.5 text-brand-secondary" /> Natural detail recovery
              </span>
              <span className="inline-flex items-center gap-1.5">
        ...
        {state === "success" && originalUrl && processedUrl && fileInfo && (
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <ResultSection
              ...
              resultAriaLabel="Image enhancement results"
              processedLabel="Enhanced Image"
              originalAlt="Original uploaded image before enhancement"
              processedAlt="Enhanced image output with improved detail and color"
